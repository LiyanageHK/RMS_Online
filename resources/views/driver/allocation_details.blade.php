            ...
            <tbody>
                @forelse($deliveries as $delivery)
                    <tr>
                        ...
                        {{ 'DLY#' . sprintf('%03d', $delivery->delivery_id) }}</td>
                        ...
                        <td style="padding: 12px; border: 1px solid #ddd;">{{ !empty($delivery->landmark) ? $delivery->landmark : '-' }}</td>
                       ...
                       <td style="padding: 12px; border: 1px solid #ddd;">Rs. {{ number_format($delivery->total, 2) }}</td>
                        <td style="padding: 12px; border: 1px solid #ddd;">{{ $delivery->assigned_to }}</td>
                                ...
                                <form id="deleteForm-{{ $delivery->delivery_id }}" action="{{ route('driver.delete.delivery', ['delivery_id' => $delivery->delivery_id]) }}" method="POST" style="display:inline-block;">
                                    @csrf
                                    @method('DELETE')
                                    <button type="button"
